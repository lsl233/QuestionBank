...
import type { Paper, PaperFile } from '../types/index.js'
import { listPapersQuerySchema } from '../utils/validate.js'

export const papersRouter = new Hono()

function toPaper(row: QueryResultRow): Paper {
...
function toPaperFile(row: QueryResultRow): PaperFile {
  return {
    id: String(row.id),
...
papersRouter.get('/:id/files', requireMembership(), async (c) => {
  ...
  const result = await query(
    'SELECT id, paper_id, file_type, file_path, mime_type, size_bytes FROM paper_files WHERE paper_id = $1 ORDER BY file_type',
    [id]
  )
