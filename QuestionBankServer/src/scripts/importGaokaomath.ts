...
const examTypeByFolder: Record<string, string> = {
  '普通高考': '普通高考',
  '春季高考': '春季高考',
}

const regionReplacements: [RegExp, string][] = [
  [/^新高考1$/, '新高考I卷'],
  [/^新高考2$/, '新高考II卷'],
  [/^全国甲$/, '全国甲卷'],
  [/^全国乙$/, '全国乙卷'],
...
function parseFileName(fileName: string, parentExamType: string, parentDirParts: string[]): ParsedFile {
...
function sanitizeFileSystemPart(value: string): string {
  return value
    .replace(/[\\/:*?"<>|]/g, '_')
    .replace(/[,()]/g, '_')
    .replace(/\s+/g, '_')
...
function buildPaperKey(parsed: ParsedFile, usedKeys: Set<string>): string {
  const baseParts = [parsed.year, parsed.subject, parsed.examType, parsed.region]
  if (parsed.stream) baseParts.push(parsed.stream)
  ...
  if (parsed.note) {
    ...
    if (!usedKeys.has(withNote)) return withNote
  }

  // 仍冲突则使用自增序号
  let counter = 2
  while (usedKeys.has(`${baseKey}_${counter}`)) counter++
    ...
    for (const file of files) {
      ...
      try {
        ...
        const paperResult = await query(
          `INSERT INTO papers (year, exam_type, region, subject, stream, note, file_name, title)
        ...
        await query(
          `INSERT INTO paper_files (paper_id, file_type, file_path, mime_type, size_bytes)
           VALUES ($1, $2, $3, $4, $5)
           ON CONFLICT (paper_id, file_type) DO UPDATE SET
             file_path = EXCLUDED.file_path,
             mime_type = EXCLUDED.mime_type,
             size_bytes = EXCLUDED.size_bytes`,
          [paperId, FILE_TYPE, relativeFilePath, 'application/pdf', stat.size]
        )
