...
function toMembershipProduct(row) {
    return {
        id: String(row.id),
...
async function recomputeMembership(userId) {
    const result = await query(`SELECT t.product_id, mp.is_permanent, t.expires_at
    ...
    await query(`INSERT INTO user_memberships (user_id, is_active, expires_at, last_product_id, updated_at)
     VALUES ($1, $2, $3, $4, NOW())
     ON CONFLICT (user_id) DO UPDATE SET
       is_active = EXCLUDED.is_active,
       expires_at = EXCLUDED.expires_at,
       last_product_id = EXCLUDED.last_product_id,
       updated_at = NOW()`, [userId, isActive, expiresAt ? expiresAt.toISOString() : null, lastProductId]);
}
membershipRouter.get('/products', async (c) => {
    const result = await query(`SELECT id, apple_product_id, name, duration_days, is_permanent
...
membershipRouter.post('/apple/verify', requireAuth(), async (c) => {
    ...
    const productResult = await query('SELECT id, duration_days, is_permanent FROM membership_products WHERE apple_product_id = $1', [transaction.productId]);
    ...
    await query(`INSERT INTO apple_transactions
