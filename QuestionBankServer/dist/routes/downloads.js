...
downloadsRouter.get('/', async (c) => {
    ...
    const result = await query(`SELECT d.id AS download_id, d.user_id, d.created_at,
     ...
     WHERE d.user_id = $1
     ORDER BY d.created_at DESC`, [user.id]);
...
downloadsRouter.post('/', async (c) => {
    ...
    await query(`INSERT INTO downloads (user_id, paper_id) VALUES ($1, $2)
     ON CONFLICT (user_id, paper_id) DO UPDATE SET created_at = NOW()`, [user.id, body.paperId]);
