...
function toStudyRecord(row, paper) {
    return {
...
studyRecordsRouter.get('/', async (c) => {
    ...
    const result = await query(`SELECT s.id AS record_id, s.user_id, s.viewed_at, s.duration_sec,
     ...
     WHERE s.user_id = $1
     ORDER BY s.viewed_at DESC`, [user.id]);
...
studyRecordsRouter.post('/', async (c) => {
    ...
    await query(`INSERT INTO study_records (user_id, paper_id, duration_sec) VALUES ($1, $2, $3)
     ON CONFLICT (user_id, paper_id) DO UPDATE SET viewed_at = NOW(), duration_sec = GREATEST(study_records.duration_sec, EXCLUDED.duration_sec)`, [user.id, body.paperId, duration]);
