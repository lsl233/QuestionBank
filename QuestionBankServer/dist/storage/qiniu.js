...
if (env.QINIU_ENABLED === 'true') {
    if (!env.QINIU_ACCESS_KEY || !env.QINIU_SECRET_KEY || !env.QINIU_BUCKET || !env.QINIU_PUBLIC_DOMAIN) {
        throw new Error('QINIU_ENABLED=true 时，必须配置 QINIU_ACCESS_KEY、QINIU_SECRET_KEY、QINIU_BUCKET 和 QINIU_PUBLIC_DOMAIN');
    }
}
...
export function isQiniuEnabled() {
 ...
 * 2. 去除开头的 /
 */
export function toQiniuKey(relativePath) {
