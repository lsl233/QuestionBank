...
final class APIService {
    ...
    /// 下载 PDF 文件到应用沙盒的 Documents 目录。
    /// - Parameters:
    ///   - fileName: 后端 /files/:name 中的 name，不含 .pdf 后缀
    ///   - progress: 下载进度回调，0.0 ~ 1.0
    /// - Returns: 本地保存的 PDF 文件 URL
    func downloadPDF(fileName: String, progress: @escaping (Double) -> Void) async throws -> URL {
        // 1. 构造远程 PDF URL，对文件名做百分号编码以支持中文/空格/特殊字符
